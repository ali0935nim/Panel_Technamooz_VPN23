# pages.py - Panel Technamooz v1.0.0.0 stable
# ══════════════════════════════════════════════════════════════════════════════
# طراحی گلاسمورفیسم فوق‌حرفه‌ای، دو زبانه (انگلیسی و فارسی)، تم تاریک و روشن
# پشتیبانی از VLESS WS، Trojan WS و XHTTP و اشتراک‌های Clash Meta و Sing-box
...
<head>
  ...
  <title>Panel Technamooz · Sign In</title>
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&family=Vazirmatn:wght@300;400;500;600;700;800&display=swap" rel="stylesheet">
  ...
  <style>
    *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    :root, [data-theme="dark"] {
      --bg: #07070a;
      --bg-radial: radial-gradient(ellipse 90% 70% at 50% -10%, rgba(225, 29, 72, 0.18), transparent 70%), radial-gradient(ellipse 70% 50% at 10% 100%, rgba(159, 18, 57, 0.14), transparent 60%);
      --card-bg: rgba(18, 18, 25, 0.72);
      --card-border: rgba(255, 255, 255, 0.08);
      --card-border-glow: rgba(225, 29, 72, 0.4);
      --text-main: #f8fafc;
      --text-sub: #94a3b8;
      --text-muted: #64748b;
      --accent: #ef233c;
      --accent-hover: #d90429;
      --accent-soft: rgba(239, 35, 60, 0.12);
      --accent-glow: 0 0 25px rgba(239, 35, 60, 0.35);
      --input-bg: rgba(8, 8, 12, 0.6);
    ...
    [data-theme="light"] {
      --bg: #f8fafc;
      --bg-radial: radial-gradient(ellipse 90% 70% at 50% -10%, rgba(114, 9, 183, 0.08), transparent 70%), radial-gradient(ellipse 70% 50% at 10% 100%, rgba(225, 29, 72, 0.06), transparent 60%);
      --card-bg: rgba(255, 255, 255, 0.84);
      --card-border: rgba(114, 9, 183, 0.14);
      --card-border-glow: rgba(114, 9, 183, 0.35);
      --text-main: #0f172a;
      --text-sub: #475569;
      --text-muted: #94a3b8;
      --accent: #7209b7;
      --accent-hover: #560bad;
      --accent-soft: rgba(114, 9, 183, 0.1);
      --accent-glow: 0 0 25px rgba(114, 9, 183, 0.25);
      --input-bg: rgba(255, 255, 255, 0.9);
      --input-border: rgba(114, 9, 183, 0.2);
      --glass-line: rgba(114, 9, 183, 0.12);
    }
    ...
    .captcha-badge {
      ...
      text-align: center;
      color: #fff;
      background: linear-gradient(135deg, rgba(239, 35, 60, 0.35), rgba(30, 30, 45, 0.8));
      border: 1px solid var(--card-border-glow);
      border-radius: 12px;
      user-select: none;
      display: flex;
    ...
    .submit-btn {
      ...
      justify-content: center;
      gap: 8px;
      box-shadow: 0 4px 20px rgba(239, 35, 60, 0.4);
      transition: all 0.2s ease;
      margin-top: 10px;
    }
    .submit-btn:hover {
      box-shadow: 0 6px 28px rgba(239, 35, 60, 0.55);
      transform: translateY(-1px);
    }
    .submit-btn:disabled {
      opacity: 0.6;
...
<body>
  ...
  <div class="top-controls">
    <button class="pill-btn" id="lang-btn" onclick="toggleLanguage()">
      <i class="ti ti-language"></i> <span id="lang-text">فارسی</span>
    </button>
    <button class="pill-btn" id="theme-btn" onclick="toggleTheme()">
    ...
    <div class="glass-card">
      <div class="brand-header">
        <div class="logo-wrapper">
          <img src="data:image/png;base64,__LOGO_B64__" alt="Technamooz">
        </div>
        <h1 class="brand-title">
          <span>Panel Technamooz</span>
          <span class="badge-stable">stable</span>
        </h1>
        <div class="version-tag">technamooz-version-1.0.0.0</div>
        <p class="brand-desc" id="i18n-sub">Enterprise VPN Config Generator & Traffic Controller</p>
      </div>

      <div class="error-alert" id="err-box">
        <i class="ti ti-alert-circle"></i>
        <span id="err-text"></span>
      </div>

      <div class="hint-box">
        <span class="hint-title" id="i18n-hint">Initial Defaults:</span>
        <span class="hint-creds" onclick="fillDefaults()" title="Click to fill">Amirparsa / Technamooz</span>
      </div>

      <form id="login-form">
        <div class="field-group">
          <label class="field-label" id="i18n-user-label">Username</label>
          <div class="input-wrapper">
            <input type="text" id="username" value="Amirparsa" required autocomplete="username" autofocus>
            <i class="ti ti-user input-icon"></i>
          </div>
        </div>

        <div class="field-group">
          <label class="field-label" id="i18n-pass-label">Password</label>
          <div class="input-wrapper">
            <input type="password" id="password" value="Technamooz" required autocomplete="current-password">
            <button type="button" class="toggle-pw-btn" onclick="togglePasswordVisibility()">
              <i class="ti ti-eye" id="pw-eye-icon"></i>
            </button>
          </div>
        </div>

        <div class="field-group">
          <label class="field-label" id="i18n-captcha-label">Security Code</label>
          <div class="captcha-row">
            <div class="captcha-badge" id="captcha-display">__CAPTCHA_CODE__</div>
            <button type="button" class="reload-captcha-btn" onclick="refreshCaptcha()" title="Reload Captcha">
              <i class="ti ti-reload" id="captcha-reload-icon"></i>
            </button>
            <div class="input-wrapper" style="width:120px;">
              <input type="text" id="captcha" class="captcha-input" maxlength="5" placeholder="CODE" required autocomplete="off">
            </div>
          </div>
        ...
        <button type="submit" class="submit-btn" id="submit-btn">
          ...
          <span id="i18n-btn-text">Sign In to Dashboard</span>
        </button>
      </form>

      <div class="login-footer">
        <div class="login-footer-text" id="i18n-dev-by">
          Developed by <strong>Technamooz Team</strong> (Managed by amirparsa)
        </div>
        <div class="social-links">
          <a href="https://youtube.com/@technamooz" target="_blank" class="social-link" title="YouTube"><i class="ti ti-brand-youtube"></i></a>
          <a href="https://t.me/technamooz" target="_blank" class="social-link" title="Telegram Channel"><i class="ti ti-brand-telegram"></i></a>
          <a href="https://instagram.com/technamooz" target="_blank" class="social-link" title="Instagram"><i class="ti ti-brand-instagram"></i></a>
          <a href="https://twitter.com/technamooz" target="_blank" class="social-link" title="Twitter / X"><i class="ti ti-brand-x"></i></a>
          <a href="https://github.com/technamooz" target="_blank" class="social-link" title="GitHub"><i class="ti ti-brand-github"></i></a>
          <a href="https://technamooz.ir" target="_blank" class="social-link" title="Official Website"><i class="ti ti-world"></i></a>
        </div>
      </div>
    </div>
  </div>
  ...
  <script>
    let currentLang = localStorage.getItem('technamooz_lang') || 'en';
    let currentTheme = localStorage.getItem('technamooz_theme') || 'dark';

    const translations = {
      en: {
        title: "Panel Technamooz · Sign In",
        sub: "Enterprise VPN Config Generator & Traffic Controller",
        hint: "Initial Defaults:",
        userLabel: "Username",
        passLabel: "Password",
        captchaLabel: "Security Code",
        btnText: "Sign In to Dashboard",
        loggingIn: "Signing in...",
        devBy: "Developed by <strong>Technamooz Team</strong> (Managed by amirparsa)",
        langBtn: "فارسی",
      },
      fa: {
        title: "پنل تکناموز · ورود به سیستم",
        sub: "پنل حرفه‌ای ساخت و مدیریت کانفیگ‌های امن و پرسرعت",
        hint: "مشخصات پیش‌فرض ورود:",
        userLabel: "نام کاربری مدیر",
        passLabel: "رمز عبور",
        captchaLabel: "کد امنیتی (کپچا)",
        btnText: "ورود به داشبورد مدیریت",
        loggingIn: "در حال بررسی و ورود...",
        devBy: "توسعه‌یافته توسط <strong>تیم Technamooz</strong> (با مدیریت amirparsa)",
        langBtn: "English",
      }
    };
    ...
    function applyTheme(theme) {
      ...
      localStorage.setItem('technamooz_theme', theme);
    }
    ...
    function applyLanguage(lang) {
      currentLang = lang;
      localStorage.setItem('technamooz_lang', lang);
      ...
      document.getElementById('i18n-user-label').textContent = t.userLabel;
      document.getElementById('i18n-pass-label').textContent = t.passLabel;
      document.getElementById('i18n-captcha-label').textContent = t.captchaLabel;
      document.getElementById('i18n-btn-text').textContent = t.btnText;
      document.getElementById('i18n-dev-by').innerHTML = t.devBy;
    ...
    function fillDefaults() {
      document.getElementById('username').value = 'Amirparsa';
      document.getElementById('password').value = 'Technamooz';
    }

    async function refreshCaptcha() {
      const icon = document.getElementById('captcha-reload-icon');
      icon.style.animation = 'spin 0.6s linear infinite';
      try {
        const res = await fetch('/api/captcha');
        const data = await res.json();
        document.getElementById('captcha-display').textContent = data.captcha_code;
    ...
    document.getElementById('login-form').addEventListener('submit', async (e) => {
      ...
      const btnText = document.getElementById('i18n-btn-text');

      errBox.classList.remove('show');
      btn.disabled = true;
      const originalText = btnText.textContent;
      btnText.textContent = translations[currentLang].loggingIn;
      ...
      try {
        const res = await fetch('/api/login', {
          ...
          body: JSON.stringify({
            ...
            captcha_code: document.getElementById('captcha').value.trim(),
          })
        });

        if (!res.ok) {
          const errData = await res.json().catch(() => ({}));
          throw new Error(errData.detail || (currentLang === 'fa' ? 'اطلاعات ورود نادرست است' : 'Login failed'));
        }
...
/* Panel Technamooz v1.0.0.0 stable — premium dark red / black visual system */
:root{
  --bg:#09090b;--bg2:#0e0e12;--panel:#121217;--panel2:#17171e;--panel3:#1d1d25;
  --border:rgba(255,255,255,.075);--border-red:rgba(225,29,72,.35);
  --accent:#e11d48;--accent-2:#fb3b62;--accent-dark:#9f1239;
  --text:#f7f7f8;--muted:#a1a1aa;--dim:#71717a;--green:#22c55e;
  --shadow:0 24px 70px rgba(0,0,0,.45);--glow:0 0 34px rgba(225,29,72,.18)
}
html,body{background:var(--bg)!important;color:var(--text)!important}
body{font-family:Vazirmatn,Tahoma,Arial,sans-serif;background-image:radial-gradient(circle at 15% 0%,rgba(225,29,72,.12),transparent 34%),radial-gradient(circle at 95% 100%,rgba(159,18,57,.10),transparent 30%)!important}
body:before{content:"";position:fixed;inset:0;pointer-events:none;background:linear-gradient(135deg,rgba(255,255,255,.018) 1px,transparent 1px);background-size:28px 28px;mask-image:linear-gradient(to bottom,black,transparent 75%);opacity:.3}
.card,.srv-panel,.modal,.panel,.stat-card,.box{background:linear-gradient(145deg,rgba(25,25,32,.96),rgba(13,13,17,.96))!important;border:1px solid var(--border)!important;box-shadow:var(--shadow)!important;border-radius:18px!important}
.card:hover,.srv-tile:hover,.stat-card:hover{border-color:var(--border-red)!important;box-shadow:var(--glow)!important;transform:translateY(-2px);transition:.2s ease}
.sidebar{background:rgba(10,10,13,.94)!important;border-left:1px solid rgba(225,29,72,.18)!important;box-shadow:-18px 0 60px rgba(0,0,0,.3)}
.logo-name,.brand-name,.mob-title{color:#fff!important;letter-spacing:-.03em}
.logo-sub,.brand-sub{color:var(--accent-2)!important;font-weight:700!important}
.logo-img,.brand-img{border-color:var(--accent)!important;box-shadow:0 0 24px rgba(225,29,72,.25)!important}
.nav-it{color:#a1a1aa!important;border-radius:12px!important;margin:3px 10px!important}
.nav-it:hover,.nav-it.active{background:linear-gradient(90deg,rgba(225,29,72,.22),rgba(225,29,72,.06))!important;color:#fff!important;border-right:3px solid var(--accent)!important}
.nav-it i,.card-title i,.tb-title i{color:var(--accent-2)!important}
.topbar{border-bottom:1px solid var(--border)!important;margin-bottom:18px!important}
.tb-title{font-weight:900!important;color:#fff!important}.tb-sub{color:var(--muted)!important}
.btn-p,.btn-primary,.btn-main{background:linear-gradient(135deg,var(--accent),var(--accent-dark))!important;border-color:transparent!important;color:#fff!important;box-shadow:0 10px 24px rgba(225,29,72,.22)!important}
.btn-p:hover,.btn-primary:hover,.btn-main:hover{background:linear-gradient(135deg,var(--accent-2),var(--accent))!important;transform:translateY(-1px)}
.btn-s,.btn-secondary{background:#202027!important;border-color:var(--border)!important;color:#f4f4f5!important}
input,select,textarea,.form-control{background:#0d0d11!important;color:#fff!important;border:1px solid #2b2b34!important;border-radius:11px!important}
input:focus,select:focus,textarea:focus{border-color:var(--accent)!important;box-shadow:0 0 0 3px rgba(225,29,72,.13)!important}
.badge,.status,.tag{border-radius:999px!important}.bg-red{background:rgba(225,29,72,.16)!important;color:#fb7185!important}
.srv-hero{background:linear-gradient(135deg,rgba(225,29,72,.2),rgba(20,20,25,.4))!important;border:1px solid var(--border-red)!important;border-radius:16px!important}
.srv-tile{background:#15151b!important;border:1px solid var(--border)!important;border-radius:14px!important}
.srv-tile-icon{color:var(--accent-2)!important;background:rgba(225,29,72,.11)!important}
.footer{color:var(--dim)!important}.footer a,.df-link{color:var(--accent-2)!important}
.login-wrap,.wrap{filter:drop-shadow(0 30px 80px rgba(0,0,0,.35))}
.login-card,.wrap .card{background:linear-gradient(145deg,rgba(24,24,30,.98),rgba(9,9,12,.98))!important;border:1px solid rgba(225,29,72,.26)!important;box-shadow:0 30px 90px rgba(0,0,0,.55),0 0 40px rgba(225,29,72,.09)!important}
.hint{background:rgba(225,29,72,.08)!important;border-color:rgba(225,29,72,.24)!important}.hint-label{color:var(--muted)!important}.hint-val{color:#fb7185!important}
.login-card h1,.wrap h1{color:#fff!important}.login-card .sub,.wrap .sub{color:var(--muted)!important}
@media(max-width:900px){.sidebar{border-left:0!important;border-bottom:1px solid rgba(225,29,72,.18)!important}.content{padding:18px!important}.card{border-radius:15px!important}}
...
.technamooz-social-card:hover { border-color: var(--accent); }
.tsc-header { display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 12px; margin-bottom: 14px; }
.tsc-left { display: flex; align-items: center; gap: 12px; }
.tsc-logo { width: 42px; height: 42px; border-radius: 12px; overflow: hidden; border: 1px solid var(--card-bh); box-shadow: 0 0 16px var(--accent-d); }
.tsc-logo img { width: 100%; height: 100%; object-fit: cover; }
.tsc-title { font-size: 14.5px; font-weight: 800; color: var(--t1); }
.tsc-sub { font-size: 11.5px; color: var(--t2); margin-top: 2px; }
.tsc-version { font-family: ui-monospace, monospace; font-size: 12px; color: var(--t3); background: rgba(0,0,0,0.2); padding: 4px 10px; border-radius: 8px; border: 1px solid var(--card-b); }
.tsc-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(130px, 1fr)); gap: 10px; }
.tsc-item { display: flex; align-items: center; gap: 8px; padding: 8px 12px; background: var(--bg2); border: 1px solid var(--card-b); border-radius: 10px; color: var(--t2); font-size: 12.5px; font-weight: 600; text-decoration: none; transition: all 0.2s ease; }
.tsc-item:hover { border-color: var(--accent); color: var(--accent); transform: translateY(-2px); box-shadow: 0 4px 14px var(--accent-d); }
.tsc-item i { font-size: 16px; }
</style>
</head>
<body>
...
<div class="mob-top">
  <div class="ml">
    <div class="mob-logo"><img src="data:image/png;base64,__LOGO_B64__" alt="Panel Technamooz"></div>
    <span class="mob-title">Technamooz</span>
  </div>
  <div class="mob-right">
...
<aside class="sidebar" id="sb">
  ...
  <div class="logo">
    <div class="logo-img"><img src="data:image/png;base64,__LOGO_B64__" alt="Panel Technamooz"></div>
    <div><div class="logo-name">Technamooz</div><div class="logo-sub">technamooz-version-1.0.0.0 <span class="badge-stable">stable</span></div></div>
  </div>
  <div class="nav-wrap">
  ...
  <div class="sb-foot">
    <button class="theme-btn" id="lang-btn" onclick="toggleLanguage()">EN</button>
...
<section class="pg on" id="pg-overview">
  ...
  <div class="dash-footer">
    <span class="df-text">Panel Technamooz v1.0.0.0 stable · technamooz-version-1.0.0.0 stable · Developed by Technamooz Team (Managed by amirparsa)</span>
    <span class="df-text">توسط تیم Technamooz · مدیریت amirparsa</span>
    <a class="df-link" href="https://t.me/technamooz_support" target="_blank"><i class="ti ti-headset"></i> پشتیبانی</a>
  </div>
</section>
  ...
  <div class="g2">
    <div class="srv-panel">
      ...
      <div class="srv-tiles">
        ...
        <div class="srv-tile"><div class="srv-tile-icon"><i class="ti ti-versions"></i></div><div class="srv-tile-text"><div class="srv-tile-label">نسخه</div><div class="srv-tile-val">v1.0.0.0 stable</div></div></div>
        ...
        <div class="srv-tile" style="grid-column:1/-1"><div class="srv-tile-icon"><i class="ti ti-device-floppy"></i></div><div class="srv-tile-text"><div class="srv-tile-label">ذخیره‌سازی و ابزارها</div><div class="srv-tile-val">Technamooz State · /data</div><div style="display:flex;gap:8px;margin-top:9px;flex-wrap:wrap"><button class="btn btn-o btn-sm" onclick="downloadBackup()"><i class="ti ti-download"></i> دانلود Backup</button><button class="btn btn-o btn-sm" onclick="exportActiveLinks()"><i class="ti ti-file-export"></i> Export لینک‌ها</button></div></div></div>
      </div>
    </div>
    <div class="pw-panel">
      <div class="pw-hero">
      ...
      <div class="pw-body">
        ...
        <div class="pw-field" style="margin-bottom:6px">
          ...
          <input class="pw-input" type="password" id="cp-new" placeholder="حداقل ۱۰ کاراکتر" oninput="checkPwStrength(this.value)">
        ...
        <div class="pw-reqs">
          <span class="pw-req" id="req-len"><i class="ti ti-circle-dashed"></i> حداقل ۱۰ کاراکتر</span>
    ...
    <div class="card credential-card"><div class="card-title"><i class="ti ti-user-cog"></i> مشخصات ورود · Login credentials</div><p class="tb-sub" style="line-height:1.8;margin-bottom:14px">نام کاربری و رمز ورود مدیر را تغییر دهید. رمز جدید حداقل ۱۰ کاراکتر باشد.</p><input class="fi" id="cred-username" placeholder="نام کاربری جدید" style="width:100%;margin-bottom:10px"><input class="fi" id="cred-current" type="password" placeholder="رمز فعلی" style="width:100%;margin-bottom:10px"><input class="fi" id="cred-new" type="password" placeholder="رمز جدید (اختیاری)" style="width:100%;margin-bottom:12px"><button class="btn btn-p" onclick="saveCredentials()"><i class="ti ti-shield-check"></i> ذخیره مشخصات</button></div>
  </div>
</section>
<section class="pg" id="pg-telegram">
  <div class="topbar"><div><div class="tb-title"><i class="ti ti-brand-telegram"></i> Telegram Bot Center</div><div class="tb-sub">مدیریت، فعال‌سازی و کنترل ربات Technamooz از داخل پنل</div></div><div class="tb-right"><span class="badge bg-green" id="tg-status">در حال بررسی...</span></div></div>
  <div class="srv-panel tg-panel">
    <div class="srv-hero"><div class="srv-hero-icon"><i class="ti ti-robot"></i></div><div class="srv-hero-text"><div class="srv-hero-domain">Technamooz Telegram Operations</div><div class="srv-hero-sub">ساخت کانفیگ، مدیریت گروه‌های ساب و کنترل کاربران از طریق ربات</div></div></div>
    <div class="cp-body" style="padding:22px"><div class="cp-row"><div class="cp-block"><div class="cp-block-label"><i class="ti ti-key"></i> Bot Token</div><input class="cp-input-full" id="tg-token" type="password" placeholder="توکن دریافتی از @BotFather"></div><div class="cp-block"><div class="cp-block-label"><i class="ti ti-users"></i> Admin IDs</div><input class="cp-input-full" id="tg-admins" placeholder="مثلاً: 123456789,987654321"><small style="color:var(--muted)">شناسه عددی ادمین‌ها را با کاما جدا کنید.</small></div></div><div style="display:flex;gap:12px;align-items:center;margin-top:18px;flex-wrap:wrap"><label class="toggle"><input id="tg-enabled" type="checkbox"><span></span> فعال‌سازی ربات</label><button class="btn btn-p" onclick="saveTelegramSettings()"><i class="ti ti-device-floppy"></i> ذخیره و اعمال</button><span id="tg-runtime" class="tb-sub"></span></div></div>
  </div>
  <div class="g2" style="margin-top:18px"><div class="card"><div class="card-title"><i class="ti ti-sparkles"></i> قابلیت‌های ربات</div><div class="sr"><span class="sr-k">ساخت wizard کانفیگ</span><span class="sr-v" style="color:var(--green-t)">فعال</span></div><div class="sr"><span class="sr-k">مدیریت لینک و گروه ساب</span><span class="sr-v" style="color:var(--green-t)">فعال</span></div><div class="sr"><span class="sr-k">Long Polling مناسب Railway</span><span class="sr-v" style="color:var(--green-t)">فعال</span></div></div><div class="card"><div class="card-title"><i class="ti ti-shield-check"></i> نکته امنیتی</div><p class="tb-sub" style="line-height:2">توکن فقط به ادمین پنل ارسال می‌شود و در رابط کاربری به‌صورت password نمایش داده می‌شود. برای پایداری بیشتر Railway، یک Volume روی /data نگه دارید و دسترسی Admin IDها را محدود کنید.</p></div></div>
</section>
<section class="pg" id="pg-support">
  ...
  <div class="srv-panel">
    <div class="srv-hero">
      ...
      <div class="srv-hero-text">
        <div class="srv-hero-domain">پشتیبانی Technamooz</div>
        <div class="srv-hero-sub"><span class="dot dg pulse"></span> راه‌های ارتباطی رسمی Technamooz</div>
      </div>
    </div>
    <div class="srv-tiles">
      <a class="srv-tile" href="https://t.me/technamooz" target="_blank" style="text-decoration:none;cursor:pointer">
        <div class="srv-tile-icon"><i class="ti ti-brand-telegram"></i></div>
        <div class="srv-tile-text"><div class="srv-tile-label">تلگرام</div><div class="srv-tile-val">@technamooz</div></div>
      </a>
      <a class="srv-tile" href="https://www.instagram.com/technamooz" target="_blank" style="text-decoration:none;cursor:pointer">
        <div class="srv-tile-icon"><i class="ti ti-brand-instagram"></i></div>
        <div class="srv-tile-text"><div class="srv-tile-label">اینستاگرام</div><div class="srv-tile-val">@technamooz</div></div>
      </a>
      <a class="srv-tile" href="https://www.youtube.com/@technamooz" target="_blank" style="text-decoration:none;cursor:pointer">
        <div class="srv-tile-icon"><i class="ti ti-brand-youtube"></i></div>
        <div class="srv-tile-text"><div class="srv-tile-label">یوتیوب</div><div class="srv-tile-val">@technamooz</div></div>
      </a>
      <a class="srv-tile" href="https://x.com/technamooz" target="_blank" style="text-decoration:none;cursor:pointer">
        <div class="srv-tile-icon"><i class="ti ti-brand-x"></i></div>
        <div class="srv-tile-text"><div class="srv-tile-label">توییتر / X</div><div class="srv-tile-val">@technamooz</div></div>
      </a>
      <a class="srv-tile" href="https://github.com/amirparsa" target="_blank" style="text-decoration:none;cursor:pointer">
        <div class="srv-tile-icon"><i class="ti ti-brand-github"></i></div>
        <div class="srv-tile-text"><div class="srv-tile-label">سازنده</div><div class="srv-tile-val">amirparsa</div></div>
      </a>
      <a class="srv-tile" href="https://technamooz.ir" target="_blank" style="text-decoration:none;cursor:pointer">
        <div class="srv-tile-icon"><i class="ti ti-world-www"></i></div>
        <div class="srv-tile-text"><div class="srv-tile-label">وب‌سایت رسمی</div><div class="srv-tile-val">Technamooz.ir</div></div>
      </a>
      <a class="srv-tile" href="https://t.me/technamooz_support" target="_blank" style="text-decoration:none;cursor:pointer">
        <div class="srv-tile-icon"><i class="ti ti-message-circle"></i></div>
        <div class="srv-tile-text"><div class="srv-tile-label">پشتیبانی مستقیم</div><div class="srv-tile-val">@technamooz_support</div></div>
      </a>
      <a class="srv-tile" href="mailto:Technamooz@gmail.com" style="text-decoration:none;cursor:pointer">
        <div class="srv-tile-icon"><i class="ti ti-mail"></i></div>
        <div class="srv-tile-text"><div class="srv-tile-label">ایمیل</div><div class="srv-tile-val">Technamooz@gmail.com</div></div>
      </a>
    </div>
  </div>
</section>
...
<script>
let isDark=localStorage.getItem('technamooz-theme')!=='light';
...
function toggleTheme(){isDark=!isDark;localStorage.setItem('technamooz-theme',isDark?'dark':'light');applyTheme(isDark)}
applyTheme(isDark);
...
function applyLanguage(lang){currentLang=lang;localStorage.setItem('technamooz-lang',lang);document.documentElement.lang=lang;document.documentElement.dir=lang==='fa'?'rtl':'ltr';const b=document.getElementById('lang-btn');if(b)b.textContent=lang==='fa'?'EN':'فا';translatePage(lang);}
function toggleLanguage(){applyLanguage(currentLang==='en'?'fa':'en');toast(currentLang==='en'?'English mode enabled':'حالت فارسی فعال شد');}
applyLanguage(currentLang);

async function renewLink(uuid) {
  try {
    const r = await authF(`/api/links/${uuid}/renew`, { method: 'POST' });
    if (!r.ok) throw new Error('Failed to renew');
    toast(currentLang === 'fa' ? 'کانفیگ با موفقیت ۳۰ روز تمدید شد' : 'Config renewed for 30 days', 'ok');
    loadLinks();
  } catch(e) {
    toast(e.message, 'err');
  }
...
async function resetLink(uuid) {
  if (!confirm(currentLang === 'fa' ? 'آیا از ریست کردن حجم مصرفی این کانفیگ مطمئن هستید؟' : 'Reset traffic usage for this config?')) return;
  try {
    const r = await authF(`/api/links/${uuid}/reset`, { method: 'POST' });
    if (!r.ok) throw new Error('Failed to reset');
    toast(currentLang === 'fa' ? 'حجم مصرفی صفر شد' : 'Traffic usage reset to zero', 'ok');
    loadLinks();
  } catch(e) {
    toast(e.message, 'err');
  }
...
async function loadTelegramSettings(){try{const r=await authF('/api/telegram/status');const d=await r.json();document.getElementById('tg-enabled').checked=d.enabled;document.getElementById('tg-admins').value=d.admin_ids||'';document.getElementById('tg-status').textContent=d.running?'ONLINE':'OFFLINE';document.getElementById('tg-runtime').textContent=d.configured?(d.admins_count+' admin(s) configured'):'Not configured';}catch(e){}}
async function saveCredentials(){const username=document.getElementById('cred-username').value.trim(),current_password=document.getElementById('cred-current').value,new_password=document.getElementById('cred-new').value;try{const r=await authF('/api/change-credentials',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username,current_password,new_password})});const d=await r.json();if(!r.ok)throw Error(d.detail||'Unable to update credentials');toast('Login credentials updated','ok');document.getElementById('cred-current').value='';document.getElementById('cred-new').value='';}catch(e){toast(e.message,'err')}}
async function saveTelegramSettings(){const token=document.getElementById('tg-token').value,admins=document.getElementById('tg-admins').value,enabled=document.getElementById('tg-enabled').checked;try{const r=await authF('/api/telegram/settings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token,admin_ids:admins,enabled})});const d=await r.json();if(!r.ok)throw Error(d.detail||'Unable to save');toast('Telegram bot settings applied','ok');document.getElementById('tg-token').value='';loadTelegramSettings();}catch(e){toast(e.message,'err')}}
...
async function downloadBackup(){
  try{
    ...
    a.href=URL.createObjectURL(blob);a.download='technamooz-backup.json';a.click();URL.revokeObjectURL(a.href);
...
async function exportActiveLinks(){
  try{
    ...
    a.href=URL.createObjectURL(blob);a.download='technamooz-active-links.txt';a.click();URL.revokeObjectURL(a.href);
  ...
  return `<div class="cfg-card ${cardCls}">
    <div class="cfg-row">
      ...
      <div class="cfg-actions">
        ...
        <button class="btn btn-sm btn-g btn-icon" onclick="navigator.clipboard.writeText('${esc(l.clash_url)}').then(()=>toast(currentLang==='fa'?'لینک Clash کپی شد':'Clash URL copied','ok'))" title="Clash Meta"><i class="ti ti-layers-intersect"></i></button>
        <button class="btn btn-sm btn-g btn-icon" onclick="navigator.clipboard.writeText('${esc(l.singbox_url)}').then(()=>toast(currentLang==='fa'?'لینک Sing-box کپی شد':'Sing-box URL copied','ok'))" title="Sing-box"><i class="ti ti-box"></i></button>
        <button class="btn btn-sm btn-g btn-icon" onclick="showQR('${esc(l.vless_link)}','${esc(l.clash_url)}','${esc(l.singbox_url)}','${esc(l.label)}')" title="QR"><i class="ti ti-qrcode"></i></button>
        <button class="btn btn-sm btn-p btn-icon" onclick="renewLink('${l.uuid}')" title="تمدید ۳۰ روزه / Renew 30D"><i class="ti ti-calendar-plus"></i></button>
        <button class="btn btn-sm btn-amb btn-icon" onclick="resetLink('${l.uuid}')" title="ریست مصرف / Reset Usage"><i class="ti ti-refresh-alert"></i></button>
...
function openEditLink(uuid){
  ...
  document.getElementById('el-port').value=l.port||443; document.getElementById('el-cleanip').value=l.clean_ip||'';
...
async function changePw(){
  ...
  if(nw.length<4){toast('حداقل ۱۰ کاراکتر','err');return}
...
<div id="cred-modal" class="modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.7);backdrop-filter:blur(8px);z-index:999;align-items:center;justify-content:center;">
  <div class="card" style="width:100%;max-width:420px;padding:26px;border-radius:18px;border:1px solid var(--card-bh);background:var(--card);">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:18px;">
      <div style="font-size:16px;font-weight:700;color:var(--t1);"><i class="ti ti-lock-cog"></i> <span data-i18n="changeCredentials">تغییر مشخصات ورود مدیر</span></div>
      <button onclick="document.getElementById('cred-modal').style.display='none'" style="background:none;border:none;color:var(--t2);font-size:18px;cursor:pointer;"><i class="ti ti-x"></i></button>
    </div>
    <div class="f-group mb12"><label class="f-label" data-i18n="newUsername">نام کاربری جدید</label><input class="f-input" id="cred-username" value="Amirparsa"></div>
    <div class="f-group mb12"><label class="f-label" data-i18n="currentPassword">رمز عبور فعلی</label><input class="f-input" type="password" id="cred-current" placeholder="رمز فعلی"></div>
    <div class="f-group mb16"><label class="f-label" data-i18n="newPassword">رمز عبور جدید</label><input class="f-input" type="password" id="cred-new" placeholder="حداقل ۶ کاراکتر"></div>
    <div style="display:flex;gap:10px;justify-content:flex-end;">
      <button class="btn btn-g" onclick="document.getElementById('cred-modal').style.display='none'" data-i18n="cancel">انصراف</button>
      <button class="btn btn-p" onclick="saveCredentials()" data-i18n="save">ذخیره تغییرات</button>
    </div>
  </div>
</div>
...
<title>Technamooz Subscription</title>
<link rel="preconnect" href="https://fonts.googleapis.com">
...
@keyframes spin{{to{{transform:rotate(360deg)}}}}
/* Technamooz technamooz-version-1.0.0.0 stable — premium dark red / black visual system */
:root{{
  --bg:#09090b;--bg2:#0e0e12;--panel:#121217;--panel2:#17171e;--panel3:#1d1d25;
  --border:rgba(255,255,255,.075);--border-red:rgba(225,29,72,.35);
  --accent:#e11d48;--accent-2:#fb3b62;--accent-dark:#9f1239;
...
<div class="wrap">
  <div class="top">
    <div class="brand">
      <div class="brand-img"><img src="data:image/png;base64,{LOGO_B64}" alt="Panel Technamooz"></div>
      <div><div class="brand-name">Technamooz</div><div class="brand-sub">technamooz-version-1.0.0.0 stable</div></div>
    </div>
  ...
  <div class="footer">پشتیبانی: <a href="https://t.me/technamooz" target="_blank">@technamooz</a> · Technamooz technamooz-version-1.0.0.0 stable</div>
</div>
<script>
const UUID_KEY='{uuid_key}';
let savedPw='';

let isDark=localStorage.getItem('technamooz-pub-theme')!=='light';
function applyTheme(dark){{
  document.documentElement.setAttribute('data-theme',dark?'dark':'light');
  document.getElementById('theme-icon').className='ti '+(dark?'ti-sun':'ti-moon');
}}
function toggleTheme(){{isDark=!isDark;localStorage.setItem('technamooz-pub-theme',isDark?'dark':'light');applyTheme(isDark)}}
applyTheme(isDark);
...
function renderContent(d){{
  ...
  window._technamoozSubUrl  = subUrl;
  window._technamoozSubName = d.name;
  window._technamoozLinks   = d.links.map(l => ({{
    vless : l.vless_link,
    ...
    <div class="sub-info">
      ...
      <div class="sub-sub-box">
        ...
        <button class="btn btn-pur" style="padding:7px 12px;font-size:10.5px"
          onclick="navigator.clipboard.writeText(window._technamoozSubUrl).then(()=>toast('لینک ساب کپی شد ✓','ok'))">
          <i class="ti ti-copy"></i> کپی لینک ساب
        </button>
        <button class="btn btn-g" style="padding:7px 12px;font-size:10.5px"
          onclick="showQR(window._technamoozSubName + ' — کل گروه', window._technamoozSubUrl)">
            ...
            <div class="cfg-bottom">
              ...
              <div class="cfg-actions">
                <button class="btn btn-p"
                  onclick="navigator.clipboard.writeText(window._technamoozLinks[${{i}}].vless).then(()=>toast('لینک کپی شد ✓','ok'))">
                  <i class="ti ti-copy"></i> کپی لینک
                </button>
                <button class="btn btn-g"
                  onclick="showQR(window._technamoozLinks[${{i}}].label, window._technamoozLinks[${{i}}].vless)">
...
function copyAllConfigs(){{
  const links=window._technamoozLinks||[];
